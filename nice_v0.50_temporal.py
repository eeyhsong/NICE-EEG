...
slice_low = 0
slice_high = 500
len_fea = slice_high - slice_low
len_fea = np.int16((len_fea-75)/5+1)*40

result_path = '/home/songyonghao/Documents/Code/NICE/results/Conf/temporal/backward/t%d-%d/' % (slice_low, slice_high)

slice_low = np.int16(slice_low / 4)
slice_high = np.int16(slice_high / 4)
model_idx = 't5'
...
class IE():
    ...
    def get_eeg_data(self):
        ...
        # set zero 
        train_data[:, :, :, :slice_low] = 0
        train_data[:, :, :, slice_high:] = 0

        test_data[:, :, :, :slice_low] = 0
        test_data[:, :, :, slice_high:] = 0

        # # set gaussian noise
        # train_noise = np.random.normal(0, 1, train_data.shape)
        # train_data[:, :, :, :slice_low] = train_noise[:, :, :, :slice_low]
        # train_data[:, :, :, slice_high:] = train_noise[:, :, :, slice_high:]

        # test_noise = np.random.normal(0, 1, test_data.shape)
        # test_data[:, :, :, :slice_low] = test_noise[:, :, :, :slice_low]
        # test_data[:, :, :, slice_high:] = test_noise[:, :, :, slice_high:]

        return train_data, train_label, test_data, test_label

        # return train_data[:,:,:,slice_low:slice_high], train_label, test_data[:,:,:,slice_low:slice_high], test_label
