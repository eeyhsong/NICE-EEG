...
result_path = '/home/songyonghao/Documents/Code/NICE/results/Conf/cat/sa/' 
model_idx = 'cat_sa'
...
class IE():
    ...
    def train(self):
                ...
                with torch.no_grad():
                    ...
                    for i, (veeg, vimg) in enumerate(self.val_dataloader):
                        ...
                        if vloss <= best_loss_val:
                            best_loss_val = vloss
                            best_epoch = e + 1
                            torch.save(self.Enc_eeg.module.state_dict(), './model/cat_sa/' + model_idx + 'Enc_eeg_sub%d.pth' % self.nSub)
                            torch.save(self.Proj_eeg.module.state_dict(), './model/cat_sa/' + model_idx + 'Proj_eeg_sub%d.pth' % self.nSub)
                            torch.save(self.Proj_img.module.state_dict(), './model/cat_sa/' + model_idx + 'Proj_img_sub%d.pth' % self.nSub)
        ...
        self.Enc_eeg.load_state_dict(torch.load('./model/cat_sa/' + model_idx + 'Enc_eeg_sub%d.pth' % self.nSub), strict=False)
        self.Proj_eeg.load_state_dict(torch.load('./model/cat_sa/' + model_idx + 'Proj_eeg_sub%d.pth' % self.nSub), strict=False)
        self.Proj_img.load_state_dict(torch.load('./model/cat_sa/' + model_idx + 'Proj_img_sub%d.pth' % self.nSub), strict=False)
