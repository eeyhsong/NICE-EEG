...
for test self-attention and graph attention
"""
...
result_path = '/home/songyonghao/Documents/Code/NICE/results/cross/ga/bs9000/test3/' 
model_idx = 'cross_ga1'
...
parser.add_argument('-batch_size', '--batch-size', default=9000, type=int,
                    metavar='N',
...
class IE():
    ...
    def get_eeg_data(self):
        ...
        for sub_idx in range(10):
            if sub_idx != self.nSub-1:
                one_train_data = np.load(self.eeg_data_path + '/sub-' + format(sub_idx+1, '02') + '/preprocessed_eeg_training.npy', allow_pickle=True)
                one_train_data = one_train_data['preprocessed_eeg_data']
                one_train_data = np.mean(one_train_data, axis=1)
                one_train_data = np.expand_dims(one_train_data, axis=1)
    ...
    def train(self):
        ...
        train_img_feature = repeat(train_img_feature, 'b f -> n b f', n=9)
        ...
        val_eeg = torch.from_numpy(train_eeg[:4860])
        val_image = torch.from_numpy(train_img_feature[:4860])

        train_eeg = torch.from_numpy(train_eeg[4860:])
        train_image = torch.from_numpy(train_img_feature[4860:])
        ...
        self.val_dataloader = torch.utils.data.DataLoader(dataset=val_dataset, batch_size=self.batch_size_val, shuffle=False)
